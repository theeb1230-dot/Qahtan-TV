  ...
  private async verifyCanonicalDomain(baseUrl: string): Promise<boolean> {
    ...
    try {
      ...
      const effectiveOrigin = (() => {
        try { return new URL(landing.url).origin; } catch { return baseUrl.replace(/\/$/, ''); }
      })();
