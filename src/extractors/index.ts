...
async function extractStreamsInternal(url: string, referer: string | undefined, depth: number, visited: Set<string>): Promise<ResolvedStream[]> {
  ...
  try {
    ...
    const scriptMedia = html.match(/https?:\/\/[^'"\s\\]+?\.(?:m3u8|mp4)[^'"\s\\]*/gi) || [];
    for (const raw of scriptMedia) {
      const mediaUrl = raw.replace(/\\\//g, '/');
      streams.push({ name: mediaUrl.toLowerCase().includes('.m3u8') ? 'Embedded HLS' : 'Embedded Video', url: mediaUrl, isM3u8: mediaUrl.toLowerCase().includes('.m3u8'), headers: { Referer: url } });
    }
    ...
    if (html.includes('eval(function(p,a,c,k,e,d)')) {
      ...
      const unpackedMedia = unpacked.match(/https?:\/\/[^'"\s\\]+?\.(?:m3u8|mp4)[^'"\s\\]*/gi) || [];
      for (const raw of unpackedMedia) {
        const mediaUrl = raw.replace(/\\\//g, '/');
        streams.push({ name: mediaUrl.toLowerCase().includes('.m3u8') ? 'Unpacked HLS' : 'Unpacked Video', url: mediaUrl, isM3u8: mediaUrl.toLowerCase().includes('.m3u8'), headers: { Referer: url } });
      }
    }

    if (depth < MAX_IFRAME_DEPTH) {
      const iframeUrls: string[] = [];
      resp.$('iframe[src]').each((_, el) => {
        if (iframeUrls.length >= MAX_IFRAMES_PER_PAGE) return;
        const iframeUrl = absoluteHttpUrl(resp.$(el).attr('src'), url);
